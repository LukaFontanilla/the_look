...
#     timeframes: [time, date, hour, time_of_day, hour_of_day, week, day_of_week_index, day_of_week]
  ...
  measure: bounce_rate {
    ...
    sql: ${count_bounces}*1.0 / (CASE WHEN ${count}*1.0 = 0 THEN NULL ELSE ${count}*1.0 END) ;;
  }
  ...
  dimension: viewed_product_id {
    type: number
    sql: CASE
        WHEN ${event_type} = 'Product' THEN CAST(SPLIT(${full_page_url}, '/')[OFFSET(ARRAY_LENGTH(SPLIT(${full_page_url}, '/'))-1)] AS INT64)
      END
       ;;
  }
  ...
  measure: session_l_9000 {
    type: number
    sql: CASE WHEN ${sessions_count} <= 9000 THEN ${sessions_count} ELSE NULL END  ;;
  }

  measure: session_m_9000 {
    type: number
    sql: CASE WHEN ${sessions_count} >= 9000 THEN ${sessions_count} ELSE NULL END  ;;
  }
