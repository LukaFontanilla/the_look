...
#        , CAST(MAX(CASE WHEN user_id in ('\\n', '\\N') THEN NULL ELSE user_id END) AS INT64) AS session_user_id
...
#     timeframes: [time, date, week, month, hour_of_day, day_of_week]
  ...
  measure: percent_bounce_sessions {
    ...
    sql: 1.0 * ${count_bounce_sessions} / (CASE WHEN ${count} = 0 THEN NULL ELSE ${count} END) ;;
  }
  ...
  measure: cart_to_checkout_conversion {
    ...
    type: number
    value_format_name: percent_2
    sql: 1.0 * ${count_purchase} / ( CASE WHEN ${count_cart_or_later} = 0 THEN NULL ELSE ${count_cart_or_later} END) ;;
  }
  ...
  measure: overall_conversion {
    ...
    type: number
    value_format_name: percent_2
    sql: 1.0 * ${count_purchase} / (CASE WHEN ${count} = 0 THEN NULL ELSE ${count} END ) ;;
  }
...
# - view: sessions_old
#   derived_table:
#     sql_trigger_value: SELECT DATE(CONVERT_TIMEZONE('UTC', 'America/Chicago', CURRENT_TIMESTAMP()))
#     distkey: user_id
...
#             WHERE ((logs.rtime) >= (DATEADD(day,-29, DATE_TRUNC('day',CURRENT_TIMESTAMP()) ))
#                   AND (logs.rtime) < (DATEADD(day,30, DATEADD(day,-29, DATE_TRUNC('day',CURRENT_TIMESTAMP()) ) )))
#             ) AS lag
