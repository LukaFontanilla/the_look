...
#   }
  ...
  dimension: order_id {
    ...
    action: {
      ...
      param: {
        name: "user_dash_link"
        value: "/dashboards/thelook_event_2::customer_lookup?Email={{ users.email._value}}"
      }
  ...
  dimension_group: returned {
    ...
    #sql: cast(CASE WHEN ${TABLE}.returned_at = "\\N" THEN NULL ELSE ${TABLE}.returned_at END as timestamp) ;;
  ...
  dimension_group: shipped {
    ...
    #sql: cast(CASE WHEN ${TABLE}.shipped_at = "\\N" THEN NULL ELSE ${TABLE}.shipped_at END as timestamp) ;;
...
########## Dynamic Sales Cohort App ##########
  ...
  measure :  filtered_total_price {
    type: sum
    sql: CASE WHEN ${created_month_num} = '2' THEN ${total_sale_price}
        ELSE 0
      END
       ;;
  }
